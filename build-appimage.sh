...
# Get the SYSTEM Python prefix (not venv)
SYSTEM_PYTHON_PREFIX=$(python3 -c "import sys; print(sys.base_prefix)")

# Copy Python binary
cp "$(which python3)" "${APPDIR}/usr/bin/"

# Copy Python standard library from SYSTEM Python
mkdir -p "${APPDIR}/usr/lib/python${PYTHON_VERSION}"
cp -r "${SYSTEM_PYTHON_PREFIX}/lib/python${PYTHON_VERSION}/"* "${APPDIR}/usr/lib/python${PYTHON_VERSION}/" 2>/dev/null || true

# Also copy lib-dynload if it exists separately
if [ -d "${SYSTEM_PYTHON_PREFIX}/lib/python${PYTHON_VERSION}/lib-dynload" ]; then
    cp -r "${SYSTEM_PYTHON_PREFIX}/lib/python${PYTHON_VERSION}/lib-dynload" "${APPDIR}/usr/lib/python${PYTHON_VERSION}/"
fi
